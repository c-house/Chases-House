...
- **Crawler control** (ADR-033, *deployed & curl-verified*): the site is kept out of search results by a Cloudflare zone-wide **`X-Robots-Tag: noindex`** Transform Rule (`http.host eq "chases.house"`) — the authoritative deindex, since GitHub Pages can't emit headers. A thin repo `robots.txt` (allows `Claude-User`) is **appended below** Cloudflare managed-robots' auto-updating AI-crawler block; the managed Content-Signal is `search=no,ai-train=no` (advisory). WAF "Block AI Bots" is **off** — it 403'd the operator's own `Claude-User` tools and the free plan gave no reliable per-UA skip; re-enable only with a verified `(http.user_agent contains "Claude-User")` Skip rule. `chases.house/robots.txt` is served **dynamically** (`Cf-Cache-Status: DYNAMIC`) — not cache-purgeable; if a repo `robots.txt` edit doesn't appear live, re-save the **Manage robots.txt** panel (AI Crawl Control) to force origin re-detection. The `X-Robots-Tag` rule + managed settings live only in the CF dashboard — recorded verbatim in ADR-033
