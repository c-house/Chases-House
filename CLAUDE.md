...
- **Crawler control** (ADR-033, *partially deployed*): thin repo `robots.txt` (allows `Claude-User`) is **appended below** Cloudflare managed-robots' auto-updating AI-crawler block — this part is live. The intended search-suppression layer — a Cloudflare zone-wide `X-Robots-Tag: noindex` Transform Rule (the only thing that actually deindexes; GitHub Pages can't emit headers) + managed Content-Signal `search=no` — is **NOT yet in place** (see ADR-033 addendum for the exact remaining steps). `chases.house/robots.txt` is served **dynamically** by managed-robots (`Cf-Cache-Status: DYNAMIC`) — it can't be cache-purged; if a repo `robots.txt` edit doesn't appear live, re-save the **Manage robots.txt** panel (Security → Settings → Bots) to force origin re-detection
