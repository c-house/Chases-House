...
- **Crawler control** (ADR-033): thin repo `robots.txt` (allows `Claude-User`) is **appended below** Cloudflare managed-robots' auto-updating AI-crawler block. Search suppression is a Cloudflare zone-wide `X-Robots-Tag: noindex` Transform Rule + WAF "Block AI Bots" + the managed Content-Signal `search=no` — *not* in `robots.txt` (can't deindex) nor GitHub Pages (can't emit headers). `chases.house/robots.txt` is served **dynamically** by managed-robots (`Cf-Cache-Status: DYNAMIC`) — it can't be cache-purged; if a repo `robots.txt` edit doesn't appear live, re-save the **Manage robots.txt** panel (Security → Settings → Bots) to force origin re-detection. These dashboard-only settings are recorded verbatim in ADR-033
