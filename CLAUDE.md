...
- **Crawler control** (ADR-033, *PARTIAL — see addendum*): a thin repo `robots.txt` (allows `Claude-User`) is **appended below** Cloudflare managed-robots' auto-updating AI-crawler block — live. WAF "Block AI Bots" is **off** (it 403'd the operator's own `Claude-User` tools; free plan gave no reliable per-UA skip — re-enable only with a verified `(http.user_agent contains "Claude-User")` Skip rule). **NOT yet done:** the Cloudflare zone-wide **`X-Robots-Tag: noindex`** Transform Rule (the authoritative deindex; GitHub Pages can't emit headers) — so the site is **not deindexed yet** — and the managed `search=no` Content-Signal (still `search=yes`). `chases.house/robots.txt` is served **dynamically** (`Cf-Cache-Status: DYNAMIC`) — not cache-purgeable; if a repo `robots.txt` edit doesn't appear live, re-save the **Manage robots.txt** panel (AI Crawl Control) to force origin re-detection. CF dashboard steps + exact remaining work recorded in ADR-033
