...
### 0. Orient
- Read `ralph/ralph-crossword/IMPLEMENTATION_PLAN.md` — understand what's done and what's next
- Study relevant source files for the task you'll pick (especially `games/sudoku/` for patterns)
...
### 2. Investigate
- Study the relevant source files for the selected task
...
### 4. Validate
- Start the dev server if not already running: `python -m http.server 3003 &`
- Use Chrome DevTools MCP to verify:
  1. `navigate_page` to `http://localhost:3003/games/crossword/`
  2. `wait_for` the page to load
  3. `take_screenshot` — visually verify the rendered output (save to `temp/screenshots/`)
  4. `list_console_messages` with `types: ["error"]` — fix ALL errors before continuing
  5. For interactive features: `take_snapshot` → `click`/`fill` → verify result
