...
- **Nav slimmed**: home page nav is **Home · Games · Cookbook · Music · Files** — five items. Blog and Links are removed (per the design session, they were `coming-soon` filler with no implementation behind them). Music stays because its `coming-soon` tag is dynamic — the inline live-check script promotes it to a real link to `dj.thewiseguy.ai` when the Cloudflare Tunnel is up, so it represents a live feature rather than filler. **Scope**: this is the home-page nav only; interior pages (Games, Cookbook, Files, individual game pages) still carry the seven-item nav until a follow-up pass extends the cleanup site-wide.
